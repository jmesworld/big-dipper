...
export const DEFAULT_MEMO = "Sent via JMES explorer";
...
export class Ledger {
      ...
      try {
        await window.keplr.experimentalSuggestChain({
          ...
          currencies: [
            {
              ...
              coinMinimalDenom: Meteor.settings.public.coins[0].denom,
              // # of decimal points to convert minimal denomination to user-facing denomination.
              coinDecimals: Math.log10(
                Meteor.settings.public.coins[0].fraction
              ),
              // (Optional) Keplr can show the fiat value of the coin if a coingecko id is provided.
              // You can get id from https://api.coingecko.com/api/v3/coins/list if it is listed.
              coinGeckoId: Meteor.settings.public.coingeckoId,
          ...
          gasPriceStep: {
            low: parseInt(
              GasPrice.fromString(
                Meteor.settings.public.ledger.gasPrice
              ).amount.toString()
            ),
            average:
              parseInt(
  ...
  static createMultiTransfer(txContext, multiSend) {
        ...
        value: MsgMultiSend.fromPartial({
          inputs: [
            {
              ...
              coins: [
                {
                  denom: "acudos",
                  amount: (totalAmountDue * 10 ** 18).toLocaleString(
                    "fullwide",
                    { useGrouping: false }
                  ),
                },
          ...
          outputs: multiSend.map((item) => ({
            ...
            coins: [
              {
                denom: "acudos",
                amount: (item.cudos * 10 ** 18).toLocaleString("fullwide", {
                  useGrouping: false,
                }),
              },
            ],
  ...
  static createSubmitProposal(txContext, proposalData, deposit) {
    ...
    let currentOptions = [
      {
        ...
        weight: ((yesOption * 10 ** 18) / 100).toLocaleString("fullwide", {
          useGrouping: false,
        }),
      },
      {
        option: VoteOption.VOTE_OPTION_ABSTAIN,
        weight: ((abstainOption * 10 ** 18) / 100).toLocaleString("fullwide", {
          useGrouping: false,
        }),
      },
      {
        option: VoteOption.VOTE_OPTION_NO,
        weight: ((noOption * 10 ** 18) / 100).toLocaleString("fullwide", {
          useGrouping: false,
        }),
      },
      {
        option: VoteOption.VOTE_OPTION_NO_WITH_VETO,
        weight: ((vetoOption * 10 ** 18) / 100).toLocaleString("fullwide", {
          useGrouping: false,
        }),
      },
    ];
    ...
    case Ledger.PROPOSAL_TYPES.PROPOSAL_TYPE_COMMUNITY_POOL_SPEND:
      return {
        ...
        value: CommunityPoolSpendProposal.encode({
          ...
          amount: [
            { amount: proposalData.poolAmount, denom: proposalData.poolDenom },
          ],
        }).finish(),
      };
  }
